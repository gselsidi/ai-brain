...
target-root `memory/` and `state/`. Existing repo-specific root instructions
are preserved outside the managed bridge block. Set `INSTALL_ROOT_AGENTS=0`
for private local-only installs that should not add or update the root bridge.
...
For subfolder installs, AI Brain writes sticky local data to target-root
`memory/`, `state/`, `specs/`, and `state/reports/`, then updates target
`.gitignore` so that data remains private by default. Treat `ai-brain/` as
replaceable framework code and those target-root generated files as the target
repo's durable AI Brain memory, specs, state, and evidence.
...
Brain source checkout without a parent target repo. Before removing generated
local artifacts, it moves non-conflicting AI Brain memory, state, reports, and
dated specs from `ai-brain/` to the target repo root.
...
user prompt
  ...
  -> memory/PROJECT_MEMORY.md context recovery when present
  ...
  -> state/sdlc_state.json and durable memory update
```
...
| Manual-copy cleanup | `make manual-copy-clean` | migrates local data, then removes nested `.git` and generated artifacts |

...
def render_bridge(*, target_root: Path, ai_brain_root: Path, data_root: Path | None = None) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "Mandatory start-of-turn rule for the primary/root orchestrator on",
            "every new user prompt in this repository:",
            "",
            ...
            "",
            "- Terse architect-engineer mode: decision, evidence, blocker, or next",
            "  action only; final: outcome, verification, risk. Assess, act, verify;",
            "  keep private reasoning private; batch safe work.",
            "- Use task-native compact handoffs: result, evidence, risk, next action.",
            "",
            "Release-window discipline:",
            "",
            "- Treat related follow-up corrections as one active release window.",
            "  Bootstrap, route, and create/update the durable spec once; use",
            "  focused checks during iteration.",
            "- Do not repeat full discovery, complete test/build/report cycles, or",
            "  commit/push/deploy for each correction. Run them once when the user",
            "  explicitly asks to finish, commit, push, deploy, release, or closes",
            "  the window.",
            "- Reopen full planning only for a material objective/boundary change or",
            "  a focused-check blocker. Immediate narrow safety checks remain allowed.",
            "",
            "Required bounded AI Brain subagent policy:",
            "",
            "- Routed roles and selected specialists are review lenses, not",
            "  automatic spawn instructions. For substantial work, spawn at least",
            "  one child when a safe independent research, audit, implementation,",
            "  test, review, or verification workstream exists. Use at most four.",
            "- If substantial work stays single-agent, record that no safe independent",
            "  workstream exists, delegation is unavailable or user-disabled, or",
            "  shared mutable state makes delegation unsafe.",
            "- Every child must use `fork_turns=\"none\"`, receive only a short task-specific summary,",
            "  and not inherit the parent conversation.",
            "  Recursive subagent spawning is prohibited.",
            "- A delegated child does not repeat steps 1-8. It must skip AI",
            "  Brain bootstrap, project memory/profile, routing catalogs,",
            "  `/goal`, prompt specs, lifecycle reports, and release gates",
            "  unless its task packet explicitly requires one of them.",
            "- The primary/root orchestrator integrates child evidence, reruns",
            "  deterministic checks, owns external release actions, and alone can",
            "  declare completion.",
            "",

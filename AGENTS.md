...
2. Read local `memory/PROJECT_MEMORY.md` if it exists. For target repos that
   install AI Brain under `ai-brain/`, read target-local
   `.ai-brain/memory/PROJECT_MEMORY.md` when the root bridge points there.
3. Read local `state/ai_brain_repo_profile.local.json` if it exists; this is
   generated by `make init-repo` from the target checkout. For subfolder
   installs, read target-local
   `.ai-brain/state/ai_brain_repo_profile.local.json` when the root bridge
   points there.
...
9. When the work changes the target repo, create or update a repo work spec
   under `.ai-brain/specs/work/` for subfolder installs, or `specs/work/` when
   AI Brain is the repo root, and keep target command, target drift,
   docs-update, and release evidence tied to it.
...
initializer populates target-local `.ai-brain/` memory, lifecycle state, repo
work specs, reports, and a machine-readable repo profile so `ai-brain/` can be
replaced without losing project context. Contracts remain AI Brain framework
contracts; do not manually copy the memory template or replace the tracked
generic framework contracts just to adopt the harness.
